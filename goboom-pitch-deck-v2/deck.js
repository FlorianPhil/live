...
(function () {
  ...
  function renderStatement(s, idx) {
    const c = s[lang] || s.en;
    const words = String(c.headline || "").split(/\s+/).filter(Boolean).map(function (w, i) {
      return '<span class="stmt-word" style="--i:' + i + '">' + esc(w) + "</span>";
    }).join(" ");
    /* Accent rule sits BELOW the copy (flex column) — never behind letters */
    return (
      '<div class="statement-stage">' +
      '<h2 class="headline xxl statement-copy">' + words + "</h2>" +
  ...
  function renderCaseTimeline(s, idx) {
    ...
    function tl(arr) {
      return (arr || []).map(function (it, i) {
        ...
        return (
          '<div class="tl-item build" style="--ti:' + i + '"><div class="yr">' + esc(yr) +

# Go Boom · EU Distributor 2.0 — Pitch Deck (V1 safe)

Bilingual HTML keynote for Bastien / Guillaume.

## Live URL (share this)

https://florianphil.github.io/live/goboom-pitch-deck/

## Open locally

```bash
cd working-folder/goboom-pitch-deck
python3 -m http.server 8799
# http://127.0.0.1:8799/
```

Cinematic overdrive lives in `../goboom-pitch-deck-v2/` (separate experiment).

## Edit copy

**Only edit `content.js`.** It has a full guide at the top.
Refresh the browser after saving.

## Controls

← → / Space / Scroll · F fullscreen · L language · C hide chrome

## Brand

Dragon Orange `#FF841A` · Black `#050505` · Barlow Condensed + Barlow  
Official Go Boom logo asset on title (stacked mark — GO is intentionally shorter than BOOM) · Novium case study only

## Downloads

- Deck zip: https://florianphil.github.io/live/goboom-pitch-deck.zip
- V2 cinematic: https://florianphil.github.io/live/goboom-pitch-deck-v2/
- V2 zip: https://florianphil.github.io/live/goboom-pitch-deck-v2.zip
